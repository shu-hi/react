...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const windowWidth:number=window.innerWidth<600?window.innerWidth:800;
      console.log(windowWidth);
      const res2 = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/tile2map?x=${data.x}&y=${data.y}&zoom_level=${data.zoom_level}&window_width=${windowWidth}`
      );
      const html = await res2.text();
  ...
  return (
    <div>
      ...
      <div style={{display:'flex'}}>
        <div className='mesh-ex first-mesh-ex'>
          {population_rate.data[0][1]}
          <div className='mesh-ex'>
            <p className='slash'>/</p>
